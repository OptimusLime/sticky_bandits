...
TARGETS="api.stickerbox.com ws.stickerbox.com"
...
# Redirect stickerbox domains to our gateway
address=/api.stickerbox.com/$GATEWAY_IP
...
RESOLVED=$(dig +short api.stickerbox.com @127.0.0.1 2>/dev/null | head -1 || true)
if [[ "$RESOLVED" == "$GATEWAY_IP" ]]; then
    ok "DNS override working: api.stickerbox.com -> $GATEWAY_IP"
else
